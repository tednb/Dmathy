...
```{r}
library(data.table)
series <- t(fread("~/data/infinium/liver_sample/fat_liver/GSE61454/series.txt",header = F))
colnames(series) <- series[1,]
series <- series[-1,]
GPL <- fread("~/data/infinium/GPL13534/GPL13534-11288.txt", sep = "\t", header = T)
rgset <- read.metharray.exp("~/data/infinium/liver_sample/fat_liver/GSE61454/idat")
