...
library(ggsignif)
load("~/data/infinium/liver_sample/79_livers(25_healthy)/GSE61258/GSE61258_qc.Rd")
df_mix <-  data.frame(age_79_all$All, age_79$Chol, age_79$Hep)
#df_mix<-df_mix-qc.o@s$age
for(s in 1:ncol(df_mix)){
  df_mix[,s] <- resid(lm(df_mix[,s]~qc.o@s$age))
}
...
df_mix$hep_f <- qc.o@ctf.o[[1]][,3]
df_mix$Chol_f <- qc.o@ctf.o[[1]][,1]
#horvath
H_age<-iageF(PredTage(qc.o@m))
acc_h <- resid(lm(H_age~qc.o@s$age))
...
pdf("bmi-aa_gse61258.pdf",width = 8,height = 8)
par(mfrow=c(2,2))
plot(df_mix_c$bmi,df_mix_c$Hep,xlab = "BMI",ylab = "Age acceleration",main = "Hep")
abline(lm(df_mix_c$Hep~df_mix_c$bmi))
text(x=55,y=25,label = paste0("PCC: ",format(cor(df_mix_c$bmi,df_mix_c$Hep),digits=2),", ","P = ",format(cor.test(df_mix_c$bmi,df_mix_c$Hep)$p.value,digits =1)))
plot(df_mix_c$bmi,df_mix_c$Chol,xlab = "BMI",ylab = "Age acceleration",main = "Chol")
abline(lm(df_mix_c$Chol~df_mix_c$bmi))
text(x=55,y=-40,label = paste0("PCC: ",format(cor(df_mix_c$bmi,df_mix_c$Chol),digits=2),", ","P = ",format(cor.test(df_mix_c$bmi,df_mix_c$Chol)$p.value,digits =1)))
plot(df_mix_c$bmi,df_mix_c$All,xlab = "BMI",ylab = "Age acceleration",main = "All")
abline(lm(df_mix_c$All~df_mix_c$bmi))
text(x=55,y=-15,label = paste0("PCC: ",format(cor(df_mix_c$bmi,df_mix_c$All),digits=2),", ","P = ",format(cor.test(df_mix_c$bmi,df_mix_c$All)$p.value,digits =1)))
plot(df_mix_c$bmi,df_mix_c$Hov,xlab = "BMI",ylab = "Age acceleration",main = "Hov")
abline(lm(df_mix_c$Hov~df_mix_c$bmi))
text(x=55,y=-15,label = paste0("PCC: ",format(cor(df_mix_c$bmi,df_mix_c$Hov),digits=2),", ","P = ",format(cor.test(df_mix_c$bmi,df_mix_c$Hov)$p.value,digits =1)))
dev.off()
```
...
ylim(-30,30)+
# add p-value
annotate("text",x = "All",y = 26,label = format(p[1],digits = 2),size = 3)+
annotate("text",x = "Chol",y = 26,label = format(p[2],digits = 2),size = 3)+
annotate("text",x = "Hep",y = 26,label = format(p[3],digits = 2),size = 3)+
annotate("text",x = "Hov",y = 26,label = format(p[4],digits = 2),size = 3)
dev.off()
...
acc_nor <- list()
for (i in 1:length(re_epic[[2]]$age_pre)){
# calculate the age acceleration
# acc_pre[[cell]] <- age_pre[[cell]] - qc_te.o@s$age
acc_nor[[i]] <-re_epic[[2]]$age_pre[[i]] - he.o@s$age # resid method
}
...
acc$Hov<-acc_h
ctf <- rbind(he.o@ctf.o[[1]],qc_te.o@ctf.o[[1]])
bmi <- c(he.o@s$bmi,qc_te.o@s$bmi)
ill_nor <- rep("0",210)
ill_de <- qc_te.o@s$desease
ill <- c(ill_nor,ill_de)

acc$ill <- as.numeric(ill)
acc$bmi <- bmi
acc$hep_f <- ctf[,3]


acc_lm<-lm(Hep~ill + hep_f + bmi,data = acc)
acc_lm3 <- lm(acc$hep_f~acc$ill)
acc_lm2 <- lm(Hep~ill,data = acc)
acc_lm1 <- lm(Hep~hep_f+bmi,data = acc)
res_ill <- resid(acc_lm1)
boxplot(acc$hep_f~acc$ill,xlab = "Grades",ylab = "hepatocyte fraction")
boxplot(acc$Chol_f~acc$ill,xlab = "Grades",ylab = "cholangiocyte fraction")
boxplot(resid(acc_lm1)~acc$ill,xlab = "Grades",ylab = "Residuals",main = "resid(lm(acc ~ fraction(hepatocyte))")

acc_lm4<-lm(ages~ill + hep_f,data = acc)
acc_lm5 <- lm(ages~hep_f,data = acc)
boxplot(resid(acc_lm5)~acc$ill,xlab = "Grades",ylab = "Residuals",main = "resid(lm(ages ~ fraction(hepatocyte))")
acc$Hep <- res_ill
...
rmse_df <- data.frame(rmse_40,rmse_25,rmse_hep,rmse_obese,rmse_cd4,rmse_mono,rmse_colon,rmse_pros,rmse__kid)
pcc_df <- data.frame(pcc_40,pcc_25,pcc_hep,pcc_obese,pcc_cd4,pcc_mono,pcc_colon,pcc_pros,pcc_kid)
mae_df <- data.frame(mae_40,mae_25,mae_hep,mae_obese,mae_cd4,mae_mono,mae_colon,mae_pros,mae_kid)
# add rownames
library(tidyr)
library(dplyr)
...
rmse_df <-  rmse_df %>% 
  ...
  mutate(dataset = factor(dataset,levels = c("rmse_40","rmse_25","rmse_hep","rmse_obese","rmse_cd4","rmse_mono","rmse_colon","rmse_pros","rmse__kid"),labels = c("Normal (n=40)","Normal (n=25)","Cultures (n=55)","Obese (n=67)","CD4T (n=214)","Monocyte (n=1202)","Colon (n=96)","Prostate (n=51)","Kidney (n=93)")))
rownames(pcc_df) <- c("Hep","Chol","All","Hov")
pcc_df <- pcc_df %>%
  tibble::rownames_to_column(var = "clock") %>% 
  gather(key = "dataset",value = "pcc",2:10) %>% 
  mutate(dataset = factor(dataset,levels = c("pcc_40","pcc_25","pcc_hep","pcc_obese","pcc_cd4","pcc_mono","pcc_colon","pcc_pros","pcc_kid"),labels = c("Normal (n=40)","Normal (n=25)","Cultures (n=55)","Obese (n=67)","CD4T (n=214)","Monocyte (n=1202)","Colon (n=96)","Prostate (n=51)","Kidney (n=93)")))
rownames(mae_df) <- c("Hep","Chol","All","Hov")
mae_df <- mae_df %>%
  tibble::rownames_to_column(var = "clock") %>% 
  gather(key = "dataset",value = "mae",2:10) %>% 
  mutate(dataset = factor(dataset,levels = c("mae_40","mae_25","mae_hep","mae_obese","mae_cd4","mae_mono","mae_colon","mae_pros","mae_kid"),labels = c("Normal (n=40)","Normal (n=25)","Cultures (n=55)","Obese (n=67)","CD4T (n=214)","Monocyte (n=1202)","Colon (n=96)","Prostate (n=51)","Kidney (n=93)")))
```

```{r}

pdf("rmse.pdf",width = 3,height = 4)
ggplot(rmse_df,aes(x = dataset,y = rmse,fill = clock))+

  # add a grey background for the first four datasets
  geom_bar(stat = "identity",position = "dodge")+
  labs(x = "Datasets",y = "RMSE")+
  # set the color of different clocks
  
  scale_fill_manual(values = c("Hep" = "#F4B183","Chol" = "#A9D18E","All" = "#696969"))+
  # hide the title of legend
  guides(fill = guide_legend(title = NULL))+
  # add a title
  ggtitle("Root Mean Square error")+#put the lengnd on the right corner in the plot,平行排列
  theme_bw()+
   #white background,
  theme(panel.background = element_rect(fill = "white",colour = "white"))+
  # hide grid lines
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank())+
  # add a grey background for the first 4 dataset
  geom_rect(xmin = 0.55,xmax = 4.45,ymin = 0,ymax = -5,fill = "#F0EAD6",alpha = 0.1)+
  geom_rect(xmin = 4.55,xmax = 6.45,ymin = 0,ymax = -5,fill = "#FFB6C1",alpha = 0.1)+
  geom_rect(xmin = 6.55,xmax = 7.45,ymin = 0,ymax = -5,fill = "#E6E6FA",alpha = 0.1)+
  geom_rect(xmin = 7.55,xmax = 8.45,ymin = 0,ymax = -5,fill = "#D3D3D3",alpha = 0.1)+
  geom_rect(xmin = 8.55,xmax = 9.45,ymin = 0,ymax = -5,fill = "#DEB887",alpha = 0.1)+
  theme(axis.text.x = element_text(angle = 45,hjust = 1))
dev.off()
# draw barplot for pcc using ggplot2
pdf("pcc.pdf",width = 5,height = 4)
ggplot(pcc_df,aes(x = dataset,y = pcc,fill = clock))+
  geom_bar(stat = "identity",position = "dodge")+
  labs(x = "Datasets",y = "PCC")+
  scale_fill_manual(values = c("Hep" = "#F4B183","Chol" = "#A9D18E","All" = "#696969","Hov" = "purple"))+
  guides(fill = guide_legend(title = NULL))+
  #ggtitle("Pearson Correlation Coefficient")+
  theme_bw()+theme(legend.position = c(0.5,0.9),legend.direction = "horizontal")+
  theme(panel.background = element_rect(fill = "white",colour = "white"))+
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank())+
  geom_rect(xmin = 0.55,xmax = 4.45,ymin = 0,ymax = -1,fill = "#F0EAD6",alpha = 0.1)+
  geom_rect(xmin = 4.55,xmax = 6.45,ymin = 0,ymax = -1,fill = "#FFB6C1",alpha = 0.1)+
  geom_rect(xmin = 6.55,xmax = 7.45,ymin = 0,ymax = -1,fill = "#E6E6FA",alpha = 0.1)+
  geom_rect(xmin = 7.55,xmax = 8.45,ymin = 0,ymax = -1,fill = "#D3D3D3",alpha = 0.1)+
  geom_rect(xmin = 8.55,xmax = 9.45,ymin = 0,ymax = -1,fill = "#DEB887",alpha = 0.1)+
  theme(axis.text.x = element_text(angle = 45,hjust = 1))+
  theme(legend.position = "none")
dev.off()
# draw barplot for mae
pdf("mae.pdf",width = 5,height = 4)
ggplot(mae_df,aes(x = dataset,y = mae,fill = clock))+
  ...
  theme(panel.background = element_rect(fill = "white",colour = "white"))+
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank())+
  geom_rect(xmin = 0.55,xmax = 4.45,ymin = 0,ymax = -3,fill = "#F0EAD6",alpha = 0.1)+
  geom_rect(xmin = 4.55,xmax = 6.45,ymin = 0,ymax = -3,fill = "#FFB6C1",alpha = 0.1)+
  geom_rect(xmin = 6.55,xmax = 7.45,ymin = 0,ymax = -3,fill = "#E6E6FA",alpha = 0.1)+
  geom_rect(xmin = 7.55,xmax = 8.45,ymin = 0,ymax = -3,fill = "#D3D3D3",alpha = 0.1)+
  geom_rect(xmin = 8.55,xmax = 9.45,ymin = 0,ymax = -3,fill = "#DEB887",alpha = 0.1)+
  theme(axis.text.x = element_text(angle = 45,hjust = 1))+
  #hide legend
  theme(legend.position = "none")
dev.off()
